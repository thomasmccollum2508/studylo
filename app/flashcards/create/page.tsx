...
export default function CreateFlashcardSet() {
  ...
  const handleCreate = async (startPractice: boolean = false) => {
    if (!title.trim()) {
    ...
    try {
      // Generate unique ID for manual flashcard set
      const flashcardSetId = `manual-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      ...
      // Save flashcards to localStorage (without study set)
      localStorage.setItem(`flashcards-${flashcardSetId}`, JSON.stringify(formattedCards));

      // Save metadata for the flashcard set
      const metadata = {
        id: flashcardSetId,
        title: title.trim(),
      ...
      localStorage.setItem(`flashcard-metadata-${flashcardSetId}`, JSON.stringify(metadata));

      // Save description/content if provided
      if (description.trim()) {
        localStorage.setItem(`note-content-${flashcardSetId}`, description.trim());
      }

      if (startPractice) {
        // For manual flashcards, navigate to a special route or use the flashcard ID
      ...
      alert(error.message || 'Failed to create flashcard set. Please try again.');
    } finally {
      setCreating(false);
    }
  };
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 transition-colors duration-300">
      ...
      <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-8 py-4 transition-colors duration-300">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <Link href="/flashcards" className="flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={creating}
              className="px-6 py-2.5 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Create
            </button>
            <button
              ...
              disabled={creating}
              className="px-6 py-2.5 bg-[#0055FF] hover:bg-[#0044CC] text-white rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Create and practice
            </button>
          </div>
