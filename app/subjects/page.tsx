    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col">
        ...
        <nav className="flex-1 px-3">
          ...
          <Link href="/subjects" className="flex items-center gap-3 px-3 py-2.5 mb-1 bg-blue-50 text-blue-600 rounded-lg font-medium">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-8">
          <div className="max-w-6xl">
            ...
            <p className="text-gray-600 mb-6">Organize your studies by subject</p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { name: 'Chemistry', sets: 8, color: 'green' },
                { name: 'History', sets: 5, color: 'orange' },
                { name: 'Mathematics', sets: 6, color: 'blue' },
                { name: 'Languages', sets: 4, color: 'purple' },
                { name: 'Biology', sets: 7, color: 'green' },
                { name: 'Physics', sets: 3, color: 'blue' },
                { name: 'Literature', sets: 4, color: 'purple' },
                { name: 'Computer Science', sets: 5, color: 'blue' },
              ].map((subject, index) => (
                <div key={index} className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                  <div className={`w-12 h-12 bg-${subject.color}-50 rounded-lg flex items-center justify-center mb-4`}>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M5 7C5 5.89543 5.89543 5 7 5H17C18.1046 5 19 5.89543 19 7V17C19 18.1046 18.1046 19 17 19H7C5.89543 19 5 18.1046 5 17V7Z" stroke={subject.color === 'green' ? '#10B981' : subject.color === 'orange' ? '#F97316' : subject.color === 'purple' ? '#8B5CF6' : '#0055FF'} strokeWidth="2"/>
                      <path d="M9 5V19" stroke={subject.color === 'green' ? '#10B981' : subject.color === 'orange' ? '#F97316' : subject.color === 'purple' ? '#8B5CF6' : '#0055FF'} strokeWidth="2"/>
                    </svg>
                  </div>
                  <h3 className="font-semibold text-gray-900 mb-1">{subject.name}</h3>
                  <p className="text-sm text-gray-500">{subject.sets} study sets</p>
                </div>
              ))}
            </div>
          </div>
