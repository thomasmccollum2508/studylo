...
export default function ResultsPageContent() {
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
        ...
        <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-8 py-4 transition-colors duration-300">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link href="/ai-generator" className="px-4 py-2 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg font-medium transition-colors">
                New Analysis
              </Link>
            </div>
          </div>
      ...
      {/* Save Note Dialog - same as original, truncated for length - will copy from original */}
      {showSaveDialog && (
            ...
            {saveSuccess ? (
              <div className="text-center py-4">
                ...
                <div className="flex gap-3">
                  {savedStudySetId && (
                    <Link href={`/my-study-sets/${savedStudySetId}`} className="flex-1 px-4 py-2 bg-[#0055FF] hover:bg-[#0044CC] text-white rounded-lg font-medium transition-colors text-center" onClick={() => { setShowSaveDialog(false); setSaveSuccess(false); setNoteTitle(''); setNoteSubject(''); setUseCustomSubject(false); setSavedStudySetId(null); }}>
                      Go to Study Set
                    </Link>
                  )}
                  <button onClick={() => { setShowSaveDialog(false); setSaveSuccess(false); setNoteTitle(''); setNoteSubject(''); setUseCustomSubject(false); setSavedStudySetId(null); }} className="flex-1 px-4 py-2 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg font-medium transition-colors">
                    Close
                  </button>
                </div>
              </div>
              ...
              <>
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={noteTitle} onChange={(e) => setNoteTitle(e.target.value)} placeholder="Enter note title" className="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-black dark:text-white bg-white dark:bg-gray-700" autoFocus />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Subject (optional)</label>
                    {!useCustomSubject ? (
                      <select value={noteSubject} onChange={(e) => { if (e.target.value === '__custom__') { setUseCustomSubject(true); setNoteSubject(''); } else { setNoteSubject(e.target.value); } }} className="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-black dark:text-white bg-white dark:bg-gray-700">
                        <option value="">None</option>
                        {loadingSubjects ? <option disabled>Loading subjects...</option> : (<>{availableSubjects.map((subject) => (<option key={subject} value={subject}>{subject}</option>))}<option value="__custom__">+ Add New Subject</option></>)}
                      </select>
                    ) : (
                      <div className="space-y-2">
                        <input type="text" value={noteSubject} onChange={(e) => setNoteSubject(e.target.value)} placeholder="Enter new subject name" className="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-black dark:text-white bg-white dark:bg-gray-700" autoFocus />
                        <button type="button" onClick={() => { setUseCustomSubject(false); setNoteSubject(''); }} className="text-sm text-blue-600 dark:text-blue-400 hover:underline">← Select from existing subjects</button>
                      </div>
                    )}
                  </div>
                  {saveError && <div className="p-3 rounded-lg bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 text-sm">{saveError}</div>}
                </div>
                <div className="flex gap-3 mt-6">
                  <button onClick={() => { setShowSaveDialog(false); setNoteTitle(''); setNoteSubject(''); setSaveError(''); setUseCustomSubject(false); }} className="flex-1 px-4 py-2 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg font-medium transition-colors" disabled={saving}>Cancel</button>
                  <button onClick={async () => {
                    if (!noteTitle.trim()) { setSaveError('Please enter a title'); return; }
                    setSaving(true); setSaveError('');
                    try {
                      const supabase = createClient();
                      if (!supabase) { setSaveError('Server not configured.'); setSaving(false); return; }
                      const { data: { user }, error: authError } = await supabase.auth.getUser();
                      if (authError || !user) { setSaveError('You must be logged in to save notes'); setSaving(false); return; }
                      const { data, error } = await supabase.from('study_sets').insert({ user_id: user.id, title: noteTitle.trim(), subject: noteSubject.trim() || null, card_count: 0 }).select().single();
                      if (error || !data) { setSaveError(error?.message || 'Failed to save'); setSaving(false); return; }
                      if (summary) localStorage.setItem(`note-content-${data.id}`, summary);
                      ...
                      if (summaryForFlashcards) {
                        try {
                          ...
                          const truncatedContent = textContent.length > 50000 ? textContent.substring(0, 50000) : textContent;
                          const flashcardResponse = await fetch('/api/generate-flashcards', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: truncatedContent }) });
                          const flashcardData = await flashcardResponse.json();
                          if (flashcardData.success && flashcardData.cards) localStorage.setItem(`flashcards-${data.id}`, JSON.stringify(flashcardData.cards));
                        } catch { /* ignore */ }
                      }
                      setSaveSuccess(true); setSavedStudySetId(data.id);
                    } catch (err: unknown) {
                      setSaveError(err instanceof Error ? err.message : 'Failed to save note.');
                    } finally {
                      setSaving(false);
                    }
                  }} disabled={saving || !noteTitle.trim()} className="flex-1 px-4 py-2 bg-[#0055FF] hover:bg-[#0044CC] disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors">{saving ? 'Saving...' : 'Save'}</button>
                </div>
              </>
            )}
          </div>
